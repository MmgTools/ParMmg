...
int PMMG_build_simpleExtNodeComm( PMMG_pParMesh parmesh ) {
  ...
  int             ier,i,j,k,iel,ifac,ip,iploc,grpid;
  ...
  for ( k=0; k<next_face_comm; ++k ) {
    ...
    ext_node_comm->color_in  = ext_face_comm->color_in;
    ext_node_comm->color_out = ext_face_comm->color_out;
    ...
    ext_node_comm->nitem = 3*ext_face_comm->nitem;
    PMMG_CALLOC(parmesh,ext_node_comm->int_comm_index,ext_node_comm->nitem,
    ...
    PMMG_REALLOC(parmesh,ext_node_comm->int_comm_index,nitem_ext_comm,
                 ext_node_comm->nitem,int,"external node communicator",goto end);
  ...
  if ( !ier ) {
    ...
    if ( next_node_comm ) {
      for ( k=0; k<next_node_comm; ++k ) {
        ext_node_comm = &parmesh->ext_node_comm[k];
        PMMG_DEL_MEM(parmesh,ext_node_comm->int_comm_index,ext_node_comm->nitem,
  ...
  PMMG_DEL_MEM(parmesh,intvalues,parmesh->int_face_comm->nitem,int,
               "internal face communicator");

  PMMG_DEL_MEM(parmesh,meshId,int_face_comm->nitem,int,
               "id of the mesh associated to the face stored in intvalues");
  return ier;
}
...
int PMMG_build_intNodeComm( PMMG_pParMesh parmesh ) {
  ...
  for ( grpid=0; grpid<parmesh->ngrp; ++grpid ) {
    ...
    PMMG_CALLOC(parmesh,node2int_node_comm_index1,grp->nitem_int_node_comm,int,
                "node2int_node_comm_index1",goto end);

    PMMG_CALLOC(parmesh,node2int_node_comm_index2,grp->nitem_int_node_comm,int,
                "node2int_node_comm_index2",goto end);
    ...
    PMMG_REALLOC(parmesh,node2int_node_comm_index1,nitem_int_node_comm,
                 nitem_tmp,int,"node2int_node_comm_index1",
                 goto end);
    grp->nitem_int_node_comm = nitem_int_node_comm;

    PMMG_REALLOC(parmesh,node2int_node_comm_index2,nitem_int_node_comm,
  ...
  if ( !ier ) {
    ...
    for ( grpid=0; grpid<parmesh->ngrp; ++grpid ) {
      ...
      PMMG_DEL_MEM(parmesh,node2int_node_comm_index1,grp->nitem_int_node_comm,
                   int,"node2int_node_comm_index1");
      PMMG_DEL_MEM(parmesh,node2int_node_comm_index2,grp->nitem_int_node_comm,
                   int,"node2int_node_comm_index2");
    }
  }
...
int PMMG_build_completeExtNodeComm( PMMG_pParMesh parmesh ) {
  ...
  do {
    ...
    MPI_CHECK( MPI_Allreduce(&loc_update,&glob_update,1,MPI_LOGICAL,MPI_LOR,
        ...
        if ( val1_i == rank ) {
          ...
          if ( nitem_ext_comm[val1_j] == ext_node_comm->nitem ) {
            ...
            PMMG_REALLOC(parmesh,ext_node_comm->int_comm_index,
                         (int)((1.+PMMG_GAP)*ext_node_comm->nitem),
                         ext_node_comm->nitem,int,
                         "external communicator",goto end);
            ext_node_comm->nitem = (int)((1.+PMMG_GAP)*ext_node_comm->nitem);
  ...
  for ( k=0; k<parmesh->next_node_comm; ++k ) {
    ext_node_comm = &parmesh->ext_node_comm[k];
    if ( ext_node_comm->color_out < 0 ) {
      /* Empty communicator */
      assert ( !ext_node_comm->nitem );
      continue;
    }
    else {
      assert ( ext_node_comm->nitem );
  ...
  PMMG_REALLOC(parmesh,ext_node_comm,
