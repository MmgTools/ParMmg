...
#    >     -s <size>   Defines PMMG_INT  kind (4 or 8 usually)
#
#   Authors:
...
my $int      = 0;
my $fichier;
#my $format = "PMMG_INTEGER, PARAMETER :: %-30s = %d";
my $format = "#define %-30s %d";
my $formatbyval = "#define %-30s \%val(%d)";
my $definebyval = "#define PMMG_ARG_%-30s \%val(%d)\n";
my %opts;
...
sub printTab # ($chaine, $tabcount, $comm)
{
    ...
    if ($comm == 0)
    {
        if ($int != 0)
        {
            $chaine =~ s/PMMG_INTEGER,/INTEGER(KIND=$int),/g;
        }
        else
        {
            $chaine =~ s/PMMG_INTEGER,/INTEGER,/g;
        }
        if ($real != 0)
        {
    ...
    foreach my $line ( <APIc> )
    {
        if ($startcom == 0)
        {
            if ($startenum == 0)
            {
                ...
                elsif ($line =~ /\#define PMMG_ARG_(\w*)\s+(.*)/)
                {
        ...
        else
        {
            ...
            elsif($line =~ /(.*)/)
            {
                ...
                if ($line =~ /ParMmg's constants/)
                {
                    ...
                    $chaine2 .= "!\n";
                    $chaine2 .= "!   Contains:\n";
                    $chaine2 .= "!     PMMG_DATA_PTR_T - Kind to use for POINTERS\n";
                   # $chaine2 .= "!     PMMG_REAL_KIND  - Kind to use for REAL\n";
                   # $chaine2 .= "!     PMMG_INT_KIND   - Kind to use for INT\n";
                    $chaine2 .= "!\n";
                    if ($real != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: PMMG_REAL_KIND                = $real\n";
                    }
                    if ($int != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: PMMG_INT_KIND                = $int\n";
                    }
                    $tabcount --;
